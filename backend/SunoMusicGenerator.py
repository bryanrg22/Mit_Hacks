...
class SunoMusicGenerator:
    def __init__(self):
        ...
        if not self.api_key:
            raise ValueError("SUNO_API_KEY not found in environment variables or provided directly")
    ...
    def generate_music(self, prompt: str, tags: Optional[str] = None, make_instrumental: bool = False,
                       poll_interval: float = 3.0, timeout: float = 120.0) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not response.ok:
                raise requests.RequestException(f"Failed to start song generation: {response.status_code}: {response.text}")
            ...
            ready_clip = self._poll_for_clip(clip_id, poll_interval=poll_interval, timeout=timeout)
    ...
    def _poll_for_clip(self, clip_id: str, poll_interval: float = 3.0, timeout: float = 120.0) -> Dict[str, Any]:
        """
